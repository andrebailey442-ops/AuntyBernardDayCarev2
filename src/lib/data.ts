

import type { Student, Subject, Fee, User, FormDocument, Staff, StaffRole } from './types';

export const STUDENTS: Student[] = [
  {
    id: 'SID-1001',
    name: 'Liam Johnson',
    age: 4,
    dob: '2020-05-23',
    gender: 'Male',
    guardians: [{ firstName: 'Sarah', lastName: 'Johnson', relationship: 'Mother', contact: 'liam.johnson.parent@example.com', phone: '876-555-0101', occupation: 'Doctor', placeOfEmployment: 'University Hospital', workNumber: '876-555-1111', address: '123 Maple St', city: 'Kingston', state: 'St. Andrew' }],
    avatarUrl: 'https://picsum.photos/seed/101/100/100',
    imageHint: 'child smiling',
    preschool: true,
    afterCare: true,
    nursery: false,
    status: 'enrolled',
    emergencyContactName: 'Robert Johnson',
    emergencyContactPhone: '876-555-0102',
    medicalConditions: 'None',
  },
  {
    id: 'SID-1002',
    name: 'Olivia Williams',
    age: 5,
    dob: '2019-08-15',
    gender: 'Female',
    guardians: [{ firstName: 'David', lastName: 'Williams', relationship: 'Father', contact: 'olivia.williams.parent@example.com', phone: '876-555-0103', occupation: 'Engineer', address: '456 Oak Ave', city: 'Montego Bay', state: 'St. James' }],
    avatarUrl: 'https://picsum.photos/seed/102/100/100',
    imageHint: 'girl laughing',
    preschool: true,
    afterCare: false,
    nursery: true,
    status: 'enrolled',
    emergencyContactName: 'Mary Williams',
    emergencyContactPhone: '876-555-0104',
    medicalConditions: 'Peanut allergy',
  },
  {
    id: 'SID-1003',
    name: 'Noah Brown',
    age: 4,
    dob: '2020-01-30',
    gender: 'Male',
    guardians: [{ firstName: 'Jessica', lastName: 'Brown', relationship: 'Mother', contact: 'noah.brown.parent@example.com', phone: '876-555-0105', occupation: 'Teacher', address: '789 Pine Ln', city: 'Ocho Rios', state: 'St. Ann' }],
    avatarUrl: 'https://picsum.photos/seed/103/100/100',
    imageHint: 'boy playing',
    preschool: true,
    afterCare: true,
    nursery: true,
    status: 'enrolled',
    emergencyContactName: 'James Brown',
    emergencyContactPhone: '876-555-0106',
    medicalConditions: 'Asthma',
  },
  {
    id: 'SID-1004',
    name: 'Emma Jones',
    age: 3,
    dob: '2021-11-10',
    gender: 'Female',
    guardians: [{ firstName: 'Michael', lastName: 'Jones', relationship: 'Father', contact: 'emma.jones.parent@example.com', phone: '876-555-0107', address: '101 Elm Ct', city: 'Spanish Town', state: 'St. Catherine' }],
    avatarUrl: 'https://picsum.photos/seed/104/100/100',
    imageHint: 'toddler girl',
    preschool: true,
    afterCare: false,
    nursery: true,
    status: 'pending',
    emergencyContactName: 'Linda Jones',
    emergencyContactPhone: '876-555-0108',
    medicalConditions: 'None',
  },
  {
    id: 'SID-1005',
    name: 'Lucas Garcia',
    age: 5,
    dob: '2019-03-25',
    gender: 'Male',
    guardians: [{ firstName: 'Maria', lastName: 'Garcia', relationship: 'Mother', contact: 'lucas.garcia.parent@example.com', phone: '876-555-0109', address: '212 Birch Rd', city: 'Mandeville', state: 'Manchester' }],
    avatarUrl: 'https://picsum.photos/seed/105/100/100',
    imageHint: 'child portrait',
    preschool: true,
    afterCare: true,
    nursery: false,
    status: 'enrolled',
    emergencyContactName: 'Carlos Garcia',
    emergencyContactPhone: '876-555-0110',
    medicalConditions: 'None',
  },
  {
    id: 'SID-1006',
    name: 'Ava Miller',
    age: 4,
    dob: '2020-09-05',
    gender: 'Female',
    guardians: [{ firstName: 'Chris', lastName: 'Miller', relationship: 'Father', contact: 'ava.miller.parent@example.com', phone: '876-555-0111', address: '333 Cedar Dr', city: 'Negril', state: 'Westmoreland' }],
    avatarUrl: 'https://picsum.photos/seed/106/100/100',
    imageHint: 'child painting',
    preschool: true,
    afterCare: false,
    nursery: false,
    status: 'enrolled',
    emergencyContactName: 'Patricia Miller',
    emergencyContactPhone: '876-555-0112',
    medicalConditions: 'None',
  },
  {
    id: 'SID-1007',
    name: 'Mason Rodriguez',
    age: 5,
    dob: '2019-07-19',
    gender: 'Male',
    guardians: [{ firstName: 'Jennifer', lastName: 'Rodriguez', relationship: 'Mother', contact: 'mason.rodriguez.parent@example.com', phone: '876-555-0113', address: '444 Spruce Way', city: 'Port Antonio', state: 'Portland' }],
    avatarUrl: 'https://picsum.photos/seed/107/100/100',
    imageHint: 'boy running',
    preschool: true,
    afterCare: true,
    nursery: true,
    status: 'enrolled',
    emergencyContactName: 'Jose Rodriguez',
    emergencyContactPhone: '876-555-0114',
    medicalConditions: 'None',
  },
  {
    id: 'SID-1008',
    name: 'Sophia Martinez',
    age: 3,
    dob: '2021-02-12',
    gender: 'Female',
    guardians: [{ firstName: 'Robert', lastName: 'Martinez', relationship: 'Father', contact: 'sophia.martinez.parent@example.com', phone: '876-555-0115', address: '555 Willow Ave', city: 'May Pen', state: 'Clarendon' }],
    avatarUrl: 'https/picsum.photos/seed/108/100/100',
    imageHint: 'girl with book',
    preschool: true,
    afterCare: false,
    nursery: true,
    status: 'enrolled',
    emergencyContactName: 'Maria Martinez',
    emergencyContactPhone: '876-555-0116',
    medicalConditions: 'Lactose intolerant',
  },
  {
    id: 'SID-1009',
    name: 'Ethan Hernandez',
    age: 4,
    dob: '2020-04-01',
    gender: 'Male',
    guardians: [{ firstName: 'Lisa', lastName: 'Hernandez', relationship: 'Mother', contact: 'ethan.hernandez.parent@example.com', phone: '876-555-0117', address: '666 Redwood Cir', city: 'Falmouth', state: 'Trelawny' }],
    avatarUrl: 'https/picsum.photos/seed/109/100/100',
    imageHint: 'child on swing',
    preschool: true,
    afterCare: true,
    nursery: false,
    status: 'pending',
    emergencyContactName: 'Juan Hernandez',
    emergencyContactPhone: '876-555-0118',
    medicalConditions: 'None',
  },
  {
    id: 'SID-1010',
    name: 'Isabella Lopez',
    age: 5,
    dob: '2019-10-08',
    gender: 'Female',
    guardians: [{ firstName: 'Daniel', lastName: 'Lopez', relationship: 'Father', contact: 'isabella.lopez.parent@example.com', phone: '876-555-0119', address: '777 Sequoia Blvd', city: 'Black River', state: 'St. Elizabeth' }],
    avatarUrl: 'https://picsum.photos/seed/110/100/100',
    imageHint: 'girl smiling',
    preschool: true,
    afterCare: false,
    nursery: true,
    status: 'enrolled',
    emergencyContactName: 'Ana Lopez',
    emergencyContactPhone: '876-555-0120',
    medicalConditions: 'None',
  },
    {
    id: 'SID-1011',
    name: 'Mike Ross',
    age: 5,
    dob: '2019-11-12',
    gender: 'Male',
    guardians: [{ firstName: 'Rachel', lastName: 'Zane', relationship: 'Mother', contact: 'rachel.zane@example.com', phone: '876-555-0121', occupation: 'Lawyer', address: '888 Specter Litt', city: 'Kingston', state: 'St. Andrew' }],
    avatarUrl: 'https://picsum.photos/seed/111/100/100',
    imageHint: 'boy in suit',
    preschool: true,
    afterCare: true,
    nursery: false,
    status: 'enrolled',
    emergencyContactName: 'Harvey Specter',
    emergencyContactPhone: '876-555-0122',
    medicalConditions: 'Photographic memory',
  }
];

export const ARCHIVED_STUDENTS: Student[] = [
    {
        id: 'SID-9001',
        name: 'Michael Clark',
        age: 6,
        dob: '2018-03-15',
        gender: 'Male',
        guardians: [{ firstName: 'Linda', lastName: 'Clark', relationship: 'Mother', contact: 'linda.clark@example.com', phone: '876-555-0201', address: '123 Grad Lane', city: 'Kingston', state: 'St. Andrew' }],
        avatarUrl: 'https://picsum.photos/seed/901/100/100',
        imageHint: 'boy with backpack',
        status: 'graduated',
        graduationDate: '2023-06-15T12:00:00.000Z',
    },
    {
        id: 'SID-9002',
        name: 'Emily Lewis',
        age: 6,
        dob: '2018-05-20',
        gender: 'Female',
        guardians: [{ firstName: 'Paul', lastName: 'Lewis', relationship: 'Father', contact: 'paul.lewis@example.com', phone: '876-555-0202', address: '456 Alumni Ave', city: 'Mandeville', state: 'Manchester' }],
        avatarUrl: 'https://picsum.photos/seed/902/100/100',
        imageHint: 'girl in cap and gown',
        status: 'graduated',
        graduationDate: '2023-06-15T12:00:00.000Z',
    },
];

export const SUBJECTS: Subject[] = [
  { id: 'math', name: 'Math' },
  { id: 'science', name: 'Science' },
  { id: 'reading', name: 'Reading' },
  { id: 'art', name: 'Art' },
  { id: 'music', name: 'Music' },
];

export const GRADES: Grade[] = [];

export const ATTENDANCE: Attendance[] = [];

export const FEES: Fee[] = [
  { id: 'fee-1001', studentId: 'SID-1001', amount: 3000, amountPaid: 3000, status: 'Paid', plan: 'Full Payment' },
  { id: 'fee-1002', studentId: 'SID-1002', amount: 2500, amountPaid: 2500, status: 'Paid', plan: 'Two Installments' },
  { id: 'fee-1003', studentId: 'SID-1003', amount: 3000, amountPaid: 1500, status: 'Pending', plan: 'Two Installments' },
  { id: 'fee-1004', studentId: 'SID-1004', amount: 2500, amountPaid: 0, status: 'Pending', plan: 'Monthly Plan' },
  { id: 'fee-1005', studentId: 'SID-1005', amount: 3000, amountPaid: 3000, status: 'Paid', plan: 'Full Payment' },
  { id: 'fee-1006', studentId: 'SID-1006', amount: 2500, amountPaid: 2000, status: 'Pending', plan: 'Monthly Plan' },
  { id: 'fee-1007', studentId: 'SID-1007', amount: 3000, amountPaid: 0, status: 'Overdue', plan: 'Full Payment' },
  { id: 'fee-1008', studentId: 'SID-1008', amount: 2500, amountPaid: 2500, status: 'Paid', plan: 'Full Payment' },
  { id: 'fee-1009', studentId: 'SID-1009', amount: 3000, amountPaid: 500, status: 'Pending', plan: 'Monthly Plan' },
  { id: 'fee-1010', studentId: 'SID-1010', amount: 2500, amountPaid: 1250, status: 'Pending', plan: 'Two Installments' },
  { id: 'fee-1011', studentId: 'SID-1011', amount: 3000, amountPaid: 3000, status: 'Paid', plan: 'Full Payment' },
];

export const USERS: User[] = [
    {
      id: 'user-1',
      username: 'Admin',
      email: 'admin@example.com',
      role: 'Admin',
      password: 'admin',
      avatarUrl: 'https://picsum.photos/seed/admin/100/100',
      imageHint: 'person avatar'
    },
    {
        id: 'user-2',
        username: 'Teacher',
        email: 'teacher@example.com',
        role: 'Teacher',
        password: 'teacher',
        avatarUrl: 'https://picsum.photos/seed/teacher/100/100',
        imageHint: 'person avatar'
    }
];

export const PERMISSIONS: {id: string, label: string}[] = [
    { id: '/dashboard/preschool', label: 'Preschool Dashboard' },
    { id: '/dashboard/student-management', label: 'Student Management' },
    { id: '/dashboard/attendance', label: 'Attendance' },
    { id: '/dashboard/grades', label: 'Grades' },
    { id: '/dashboard/forms', label: 'Forms' },
    { id: '/dashboard/reports', label: 'Reports' },
    { id: '/dashboard/graduation', label: 'Graduation' },
    { id: '/dashboard/after-care', label: 'After-Care' },
    { id: '/dashboard/nursery', label: 'Nursery' },
    { id: '/dashboard/manage-users', label: 'Manage Users'},
    { id: '/dashboard/staff', label: 'Staff Management' },
];

export const DEFAULT_TEACHER_PERMISSIONS: string[] = ['/dashboard/preschool', '/dashboard/attendance', '/dashboard/grades'];

export const FORMS: FormDocument[] = [
    {
      id: 'f1',
      title: 'New Student Application',
      description: 'Standard application form for all new student registrations.',
      icon: 'FileText',
    },
    {
      id: 'f2',
      title: 'Medical & Consent Form',
      description: 'Emergency contacts and medical consent for treatment.',
      icon: 'HeartPulse',
    },
];

export const STAFF: Staff[] = [
    {
      id: 'staff-1',
      name: 'Eleanor Vance',
      dob: '1990-05-15',
      age: 34,
      address: '789 Oak Street, Kingston',
      roles: ['Preschool Attendant'],
      avatarUrl: 'https://i.pravatar.cc/150?u=eleanor',
      imageHint: 'woman smiling'
    },
    {
      id: 'staff-2',
      name: 'Marcus Holloway',
      dob: '1988-11-22',
      age: 35,
      address: '456 Pine Avenue, Montego Bay',
      roles: ['Aftercare Attendant', 'Nursery Attendant'],
      avatarUrl: 'https://i.pravatar.cc/150?u=marcus',
      imageHint: 'man with glasses'
    },
    {
      id: 'staff-3',
      name: 'Clara Oswald',
      dob: '1995-03-01',
      age: 29,
      address: '123 Maple Drive, Ocho Rios',
      roles: ['Nursery Attendant'],
      avatarUrl: 'https://i.pravatar.cc/150?u=clara',
      imageHint: 'woman with dark hair'
    }
];


export const JAMAICAN_PARISHES = [
    { value: 'St. Andrew', label: 'St. Andrew' },
    { value: 'St. Catherine', label: 'St. Catherine' },
    { value: 'Clarendon', label: 'Clarendon' },
    { value: 'Manchester', label: 'Manchester' },
    { value: 'St. Elizabeth', label: 'St. Elizabeth' },
    { value: 'Westmoreland', label: 'Westmoreland' },
    { value: 'Hanover', label: 'Hanover' },
    { value: 'St. James', label: 'St. James' },
    { value: 'Trelawny', label: 'Trelawny' },
    { value: 'St. Ann', label: 'St. Ann' },
    { value: 'St. Mary', label: 'St. Mary' },
    { value: 'Portland', label: 'Portland' },
    { value: 'St. Thomas', label: 'St. Thomas' },
    { value: 'Kingston', label: 'Kingston' },
];

export const CITIES_BY_PARISH: { [key: string]: string[] } = {
    'Kingston': ['Kingston', 'Port Royal', 'Trenchtown'],
    'St. Andrew': ['Stony Hill', 'Liguanea', 'Half-Way-Tree', 'Gordon Town', 'Mavis Bank'],
    'St. Catherine': ['Spanish Town', 'Portmore', 'Old Harbour', 'Linstead', 'Ewarton'],
    'Clarendon': ['May Pen', 'Chapelton', 'Rocky Point', 'Frankfield'],
    'Manchester': ['Mandeville', 'Christiana', 'Porus', 'Williamsfield'],
    'St. Elizabeth': ['Black River', 'Santa Cruz', 'Malvern', 'Junction'],
    'Westmoreland': ['Savanna-la-Mar', 'Negril', 'Grange Hill', 'Whitehouse'],
    'Hanover': ['Lucea', 'Hopewell', 'Green Island', 'Sandy Bay'],
    'St. James': ['Montego Bay', 'Cambridge', 'Adelphi', 'Catadupa'],
    'Trelawny': ['Falmouth', 'Duncans', 'Clark\'s Town', 'Ulster Spring'],
    'St. Ann': ['St. Ann\'s Bay', 'Ocho Rios', 'Brown\'s Town', 'Runaway Bay', 'Discovery Bay'],
    'St. Mary': ['Port Maria', 'Oracabessa', 'Highgate', 'Annotto Bay'],
    'Portland': ['Port Antonio', 'Buff Bay', 'Manchioneal', 'Hope Bay'],
    'St. Thomas': ['Morant Bay', 'Yallahs', 'Seaforth', 'Golden Grove'],
};
